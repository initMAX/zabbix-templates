...
<div align="center">
    <h1>
        Zabbix ClamAV Template
    </h1>
    <h4>
        Monitoring the ClamAV signature database via Zabbix Agent
    </h4>
    <br>
</div>
<br>
...
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors the freshness and integrity of the ClamAV signature database on Linux
hosts via a Zabbix Agent UserParameter. The agent returns the plain-text output of `sigtool --info`.
The template converts that output to JSON and exposes its fields through dependent items.

The build time is used to detect a stale database. The signature count and database version are
also collected, while the signature verification result raises a high-severity alert when it is
not `OK.`.

## What is monitored
- **Database freshness** - build time of the installed ClamAV signature database, with warning and
  high-severity thresholds.
- **Signature database** - signature count and ClamAV database version.
- **Database verification** - digital signature verification result (`OK` or `FAIL`).
- **Raw database information** - the parsed output remains available to dependent items.

## Requirements
- **Zabbix 7.0 or higher**
- Linux host with **Zabbix Agent or Agent 2**
- ClamAV `sigtool` installed on the monitored host
- A readable ClamAV signature database file, typically `/var/lib/clamav/daily.cld`

## Setup
1. Define the `clamav.status` UserParameter in `zabbix_agentd.conf` or
    `zabbix_agent2.conf`. It must print the output of `sigtool --info`:

```text
UserParameter=clamav.status,/usr/bin/sigtool --info /var/lib/clamav/daily.cvd
```

Check the path first. After the first update, `freshclam` usually replaces `daily.cvd` with the
incremental `daily.cld`. Restart the agent after adding the UserParameter.

2. Make sure the agent account can read the ClamAV database. On many distributions the `zabbix`
    user must be added to the `clamav` group or the file permissions must be adjusted.

3. Import [`ClamAV_by_Zabbix_agent.yaml`](./7.0/ClamAV_by_Zabbix_agent.yaml) and link it to the
    host. The host needs a Zabbix agent interface because the master item is a passive check.

4. Review the freshness thresholds before enabling alerting. The warning threshold is 3 days and
    the high-severity threshold is 7 days.

5. After the first polling interval, verify the values in **Monitoring -> Latest data**.

### Macros used
| Macro | Description | Default |
|---|---|---|
| `{$CLAMAV.INTERVAL}` | Interval for reading the ClamAV database information | `1h` |
| `{$CLAMAV.UPDATE.HIGH}` | Database age at which the high-severity alert is raised | `7d` |
| `{$CLAMAV.UPDATE.WARN}` | Database age at which the warning is raised | `3d` |
